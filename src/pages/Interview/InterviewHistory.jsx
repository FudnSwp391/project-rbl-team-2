...
import {
  ...
  ChevronDown, Sparkles, Layers, X, Trash2, ChevronLeft, ChevronRight, Loader2
} from 'lucide-react';
import {
  MOCK_HISTORY, INDUSTRIES, DIFFICULTIES, getScoreColor, getScoreLabel,
...
export default function InterviewHistory() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        console.log('[InterviewHistory] Fetched interviews from DB:', interviews);
        ...
        const mappedHistory = interviews.map(item => {
          ...
          // Attempt to find the matching industry in constants for Vietnamese name
        ...
        // Fallback to mock history if query fails (e.g. table not fully set up)
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Bạn có chắc chắn muốn xóa bài phỏng vấn này không? Hành động này không thể hoàn tác.')) return;
    setDeletingId(id);
    try {
      ...
      alert('Không thể xóa bài phỏng vấn. Vui lòng thử lại.');
    } finally {
      setDeletingId(null);
    }
  };
  ...
  const handleDownload = (item) => {
    // Navigate to result page and trigger print
    navigate(`/interview/result/${item.id}`, { state: { autoPrint: true } });
  };
                ...
                paginatedHistory.map((item, index) => (
                  <div key={item.id} className={`history-card iv-animate-slide-up iv-delay-${Math.min(index + 1, 8)}`}>
                    ...
                    <div className="history-card__actions">
                      ...
                      <button className="iv-btn iv-btn--ghost iv-btn--sm" title="Tải PDF"
                        onClick={() => handleDownload(item)}>
                        <Download size={14} />
                      </button>
                      <button className="iv-btn iv-btn--ghost iv-btn--sm" title="Xóa"
                        onClick={() => handleDelete(item.id)} disabled={deletingId === item.id}>
                        {deletingId === item.id ? <Loader2 size={14} className="iv-spin" /> : <Trash2 size={14} style={{ color: 'var(--iv-danger)' }} />}
                      </button>
                    </div>
                  </div>
                ))
